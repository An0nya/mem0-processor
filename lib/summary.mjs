...
/**
 * Returns true if any summary file exists for the given session+model pair,
 * without requiring the session slug (slug-agnostic filesystem check).
 * @param {string} sessionId
 * @param {string} modelId
 * @returns {boolean}
 */
export function hasSummary(sessionId, modelId) {
  if (!fs.existsSync(SUMMARIES_DIR)) return false;
  const modelSlug = modelId.replace(/[^a-zA-Z0-9-]/g, "-").replace(/-+/g, "-").toLowerCase();
  const idPrefix = sessionId.slice(0, 8);
  return fs.readdirSync(SUMMARIES_DIR).some(
    f => f.includes(idPrefix) && f.endsWith(`--${modelSlug}.txt`)
  );
}
...
export async function summarizeSession(transcript, model, registryEntry = null, opts = {}) {
  ...
  const thinkingPrefill = /gem/i.test(model.id) ? "<|think|>"
    : registryEntry?.reasoning?.startString === "[THINK]" ? "[THINK]"
    : null;
  const effectivePrompt = thinkingPrefill
